...
const ADJ = ['Super','Mega','Giga','Ultra','Based','Chad','Degen','Turbo','Hyper','Neon','Pixel','Quantum','Cosmic','Solar','Lunar','Baby','King','Queen','Dark','Golden','Cyber','Alpha','Sigma','Wild','Crazy','Lucky','Pump','Moon','Rocket','Ninja']
const NOUNS = ['Pepe','Doge','Cat','Frog','Ape','Rat','Duck','Whale','Shark','Fox','Wolf','Bear','Bull','Dragon','Goblin','Troll','Robot','Alien','Banana','Pizza','Rocket','Moon','Hat','Crown','Coin','Wojak','Chad','Meme','Clown','Bonk']
const now = Date.now()
const tokens = []
for (let i = 0; i < 200; i++) {
  const name = ADJ[i % ADJ.length] + ' ' + NOUNS[(i * 7) % NOUNS.length] + (i > 50 ? ' ' + (i % 99) : '')
  const symbol = (name.replace(/[^A-Za-z]/g, '').toUpperCase().slice(0, 4) + (i % 36).toString(36).toUpperCase()).slice(0, 8)
...
const out = path.join(__dirname, '..', 'data', 'tokens.json')
